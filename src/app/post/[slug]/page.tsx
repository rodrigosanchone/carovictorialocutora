...
// Helper para serializar datos de Firestore
...
export async function generateMetadata({
  ...
  const post = postSnap.exists() ? serializePost(postSnap.data()) : null;
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://carovictorialocutora.com/post/${params.slug}`,
...
export default async function PostPage({ params }: PageProps) {
  try {
    const postRef = doc(db, "posts", params.slug);
    const metaRef = doc(db, "postMeta", params.slug);

    const [postSnap, metaSnap] = await Promise.all([
      getDoc(postRef),
      getDoc(metaRef),
    ]);
      ...
      <div className="max-w-6xl mx-auto px-4 py-16 grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-12">
        <article>
          ...
          <div className="mt-10 flex gap-6 justify-center text-gray-600 dark:text-gray-300">
            <a
              href={`https://www.facebook.com/sharer/sharer.php?u=https://carovictorialocutora.com/post/${params.slug}`}
              target="_blank"
            ...
            <a
              href={`https://api.whatsapp.com/send?text=https://carovictorialocutora.com/post/${params.slug}`}
              target="_blank"
              rel="noopener noreferrer"
            ...
            <a
              href={`https://twitter.com/intent/tweet?url=https://carovictorialocutora.com/post/${params.slug}&text=${encodeURIComponent(
                post.title,
              )}`}
              target="_blank"
              rel="noopener noreferrer"
...
interface PageProps {
  params: Promise<{ slug: string }>; // 👈 importante: params es Promise
}
